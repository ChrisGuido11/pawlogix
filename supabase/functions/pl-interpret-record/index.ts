...
6. Extract numeric values (weight in lbs — convert from kg if needed, lab values) into structured data for trend tracking
7. Extract vaccination dates and medication schedules. For medications:
   ...
   - IMPORTANT: A prescription REFILL date is NOT the same as the next dose date. Refill dates (e.g., "Refill by August 16") indicate when to reorder from the pharmacy — ignore these for next_due. Only use dates that indicate when the pet should actually receive the next dose.
   - For daily medications (e.g., "Twice daily", "Once daily"), set frequency but do NOT set next_due — daily meds don't need date-based reminders.
